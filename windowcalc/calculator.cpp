...
const int buttonDist = 10;	// рассто€ние между кнопками
const int buttonBegX = 55;	// начало кнопок x
const int buttonBegY = 160;	// Ќачало кнопок y
RECT AnsRect{ 20, 10, WIDTH - 25, buttonBegY - 10 }; // пр€моугольник вывода чисел ответа
RECT TempRect{ 20, 10, WIDTH /2 - 25, buttonBegY /2 - 10 }; // пр€моугольник вывода временных чисел
...
std::string ans = "0";	// число, отображаемое в калькул€торе
std::string temp = "";	// временна€ переменна€
auto FontAns = CreateFont(50, 20, 3, 0, 3, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);	// шрифт дл€ ответа
auto FontTemp = CreateFont(20, 10, 3, 0, 3, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);	// шрифт дл€ временного значени€
...
void DrawNumbers(HDC hdc)	// прорисовка чисел в квадрате
{
	...
	// максимальное число разр€дов - 14, дальше не показывать
	std::string tempStr; // временна€ переменна€ дл€ хранени€ огромных чисел
	...
	if (ans == "Error")	// если произошла ошибка при вычислени€х
	{
		ans = "0";
		temp = "";
	}
...
LRESULT CALLBACK WinProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_COMMAND:
	{
		if (LOWORD(wparam) >= 0 && LOWORD(wparam) < 10)	// если вводитс€ число
		{
			if (ans == "0")
				ans = "";
			if (GotAns)
			{
				ans = "";
				GotAns = false;	// ввод нового числа после получени€ ответа
			}
			ans += std::to_string(LOWORD(wparam));
			PostMessage(hwnd, WM_PAINT, 0, 0);
		}
		else if (LOWORD(wparam) == del)	// если удал€етс€ цифра
		{
		...
		else if (LOWORD(wparam) == dot)	// если вводитс€ точка
		{
			if (GotAns)
			{
				GotAns = false;
		...
		else if (LOWORD(wparam) == clear_all)	// если производитс€ очистка выражени€
		{
			ans = "0";
			temp = "";
		}
		else
		{
			...
			{
				// операци€ равенства
				if (temp.size() == 0) // если строка пуста€
				{
					temp = ans;
				}
				else if (!(temp.back() >= '0' && temp.back() <='9'))
				{
					switch (temp.back())	// какое действие выполн€ть
					{
					case '+':
					{
